...
bloom_t* bloom_alloc(size_t n, size_t m)
{
	...
	size_t size = ((NUM_SUBTABLES * n * m * cell_bytes) / 4 + 1) * 4;
	B->T = malloc_or_die(size);
	memset(B->T, 0, size);

	printf(" buckets=%lu, cells=%lu, f_bits=%lu, c_bits=%lu, c_bytes=%lu, malloc size=%lfMib\n",
			 n, m, fingerprint_bits,counter_bits,cell_bytes, (double)size/1024/1024);
	size_t i;
...
unsigned int bloom_ldec(bloom_t* B, kmer_t x)
{
	const size_t bytes_per_bucket = B->m * cell_bytes;
...
unsigned int bloom_add(bloom_t* B, kmer_t x, unsigned int d)
{
	...
	for (i = 0; i < NUM_SUBTABLES; ++i) 
	{
		...
		while (c < c_end) 
		{
			...
			{
				cnt = get_cell_count(c);
				if (cnt + d < counter_mask) 
					set_cell_count(c, cnt + d);
				else 
					set_cell_count(c, counter_mask);
				return cnt + d;
			}
			else if (g == 0) 
			{
